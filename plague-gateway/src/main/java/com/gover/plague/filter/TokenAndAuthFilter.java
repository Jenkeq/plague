...
@AllArgsConstructor
@Component
public class TokenAndAuthFilter implements GlobalFilter, Ordered {
        ...
        if(ignoreList != null && !ignoreList.contains(resUri)){
            ...
            try {
                ...
                try {
                    ...
                    List<String> userAuthList = gson.fromJson(JSON.toJSONString(userInfo.get(AuthConstant.AUTHORITY_CLAIM_NAME)), List.class);
                    ...
                    builder.header(AuthConstant.GATEWAY_KEY, MD5.create().digestHex(AESUtil.encrypt(new String(payLoad.getBytes(StandardCharsets.UTF_8)), AuthConstant.AES_SALT_VALUE)));
                }catch (Exception e){
            ...
            builder.header(AuthConstant.TOKEN_INFO_KEY, Base64.encode(payLoad.getBytes(StandardCharsets.UTF_8))).build();
        }
        // 如果是免验证、免登陆的，那么只需要校验是否是通过网关的即可
        else{
            String ip = serverRequest.getId();
            builder.header(AuthConstant.REQUEST_ID, ip);
            try {
                builder.header(AuthConstant.NO_LOGIN_GATEWAY_KEY, MD5.create().digestHex(AESUtil.encrypt(String.valueOf(ip), AuthConstant.AES_SALT_VALUE))).build();
            } catch (Exception e) {
                return unAuthorized(exchange, ResultCode.ILLEGAL_ACCESS.getMsg(),ResultCode.ILLEGAL_ACCESS.getCode());
            }
        }
